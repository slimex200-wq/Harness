...
사용자 입력이 모호하면(구체적 제품명이 없으면) PD(Product Discovery) 단계를 먼저 실행:
- 트렌드 리서치 3개 병렬 → 후보 5개 생성 → 야심도 점수 계산 → 최선 선택
- commodity 제품(투두, 메모, URL 단축기) 자동 탈락
- AI 활용 제품 우선, 야심도 6+ 필수
PD → M11(기획) → M12(설계) → R1-R3(설계 리뷰) → M13(구현) → M14(빌드 검증) → R4-R9(프로덕트 리뷰) → M16(배포).
