...
## Phase 2: 프로덕트 생성 파이프라인

하네스가 완성(63/70+)되면, 그 하네스를 활용하여 실제 프로덕트를 자동 생성한다.
사용자 개입 없이 isolated-reviewer들이 품질을 보장한다.

### 전체 흐름

```
=== Phase 1: 하네스 생성 (M1~M10) ===
M1→M2→M3→M4→[사용자 검토]→M5→M6→M7→M8
  → 미달: M9→M10→M1 루프
  → 완성: Phase 2로

=== Phase 2: 프로덕트 생성 (M11~M16) ===
M11(기획)
  │
M12(설계) → brainstorming
  │
  ├──→ R1(아키텍처 리뷰)      ──┐
  ├──→ R2(실현 가능성 리뷰)    ──┤ 병렬, isolated-reviewer
  └──→ R3(스코프 리뷰)         ──┘
  │
  미달 → M12 재설계
  │
M13(구현) → writing-plans → subagent-driven-development
  │
M14(빌드 검증) → 빌드 통과, 테스트 통과
  │
  ├──→ R4(QA 리뷰)            ──┐
  ├──→ R5(보안 리뷰)           ──┤
  ├──→ R6(디자인 리뷰)         ──┤ 병렬, isolated-reviewer
  ├──→ R7(코드 리뷰)           ──┤
  ├──→ R8(퍼포먼스 리뷰)       ──┤
  └──→ R9(threat-lens 리뷰)    ──┘
  │
  미달 → M17(개선) → M13 재구현 루프
  │
M16(배포) → 완료
```
...
- **입력**: 사용자의 프로덕트 요구사항 (자연어) + 완성된 하네스 프로필
- **처리**: 요구사항을 기능 명세로 변환
  - 핵심 기능 목록
  - 사용자 스토리
  - 기술 제약 (하네스의 stack 기반)
- **출력**: 기획 문서 (PRD)
- **체크포인트**: `.checkpoints/product/M11.json`

### M12: 설계

- **입력**: M11 기획 문서
- **처리**: brainstorming 스킬을 활용한 설계
  - 아키텍처 설계
  - 데이터 모델
  - API 설계 (해당 시)
  - UI/UX 와이어프레임 (해당 시)
- **출력**: 설계 문서 (spec)
- **체크포인트**: `.checkpoints/product/M12.md`

### 설계 리뷰 (R1~R3) — isolated-reviewer

설계 문서만 받고, 기획/구현 과정은 모른 채 독립 검증.

| 리뷰어 | 관점 | 통과 기준 |
|--------|------|----------|
| R1 아키텍처 | 기술적 올바름, 패턴 적절성, 확장성 | 구조적 결함 0 |
| R2 실현 가능성 | 이 스택/제약 내 구현 가능한가 | 블로커 0 |
| R3 스코프 | YAGNI 위반, 과도한 기능, 누락된 기능 | 불필요 기능 0, 핵심 누락 0 |

**미달 시**: 리뷰어 피드백을 M12에 전달하여 재설계 (최대 2회).

### M13: 구현

- **입력**: 리뷰 통과한 설계 문서
- **처리**:
  1. writing-plans 스킬로 구현 계획 생성
  2. subagent-driven-development로 태스크별 구현
  3. 하네스의 TDD/코드 리뷰/보안 스킬이 구현 품질 보장
- **출력**: 실행 가능한 코드베이스
- **체크포인트**: `.checkpoints/product/M13.json`
...
- **처리**:
  ...
  2. 전체 테스트 실행 + 통과 확인
  3. 린트/포맷 검사
- **출력**: 빌드 결과 (pass/fail + 로그)
- **실패 시**: build-error-resolver 에이전트로 자동 수정 후 재검증
- **체크포인트**: `.checkpoints/product/M14.json`

### 프로덕트 리뷰 (R4~R9) — isolated-reviewer

구현 과정을 모르는 상태에서 결과물만 보고 독립 검증. confirmation bias 방지.

| 리뷰어 | 관점 | 통과 기준 |
|--------|------|----------|
| R4 QA | 버그, 엣지케이스, UX 결함 | 크리티컬 이슈 0 |
| R5 보안 | OWASP Top 10, 시크릿 노출, 인젝션 | 취약점 0 |
| R6 디자인 | UI 일관성, 접근성(a11y), 레이아웃 | 메이저 이슈 0 |
| R7 코드 | 구조, 패턴, 테스트 커버리지 | 커버리지 80%+ |
| R8 퍼포먼스 | 메모리 누수, N+1 쿼리, 로딩 속도 | 크리티컬 병목 0 |
| R9 threat-lens | "이 코드를 고의로 망가뜨린다면?" 4관점 | 크리티컬 위협 0 |
...
- **입력**: R4~R9 리뷰어 피드백
- **처리**:
  1. 각 리뷰어의 이슈를 카테고리별 정리
  2. 우선순위 결정 (크리티컬 > 메이저 > 마이너)
  3. 개선 사항을 구현 계획으로 변환
- **출력**: M13 재구현용 개선 계획
- **이후**: M13으로 돌아가 재구현
- **탈출 조건**: 전 리뷰어 통과 또는 개선 루프 3회
...
- **처리**:
  ...
  2. 배포 설정 (해당 시)
  3. 최종 스모크 테스트
- **출력**: 배포 가능한 프로덕트 + 완료 리포트

### 프로덕트 완료 리포트

```markdown
# Product Maker 완료 리포트
...
```json
{
  ...
  "product": {
    "enabled": true,
    "design_review_max_rounds": 2,
    "product_review_max_rounds": 3,
    "required_coverage": 80,
    "reviewers": {
      "design": ["architecture", "feasibility", "scope"],
      "product": ["qa", "security", "design", "code", "performance", "threat-lens"]
    }
  }
}
```
...
```
/harness-maker "할일 관리 모바일 앱 만들어줘"
```
