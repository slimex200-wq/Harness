...
```bash
bash lib/run.sh "<자연어 설명>"              # 완전 무인 실행
bash lib/run.sh --resume <generation>       # 체크포인트 재개
bash lib/run.sh "<설명>" --with-product     # 하네스 + 프로덕트
```

## Autonomous Operation Rules

1. **사용자 개입 금지**: 어떤 세대에서도 사용자에게 질문하지 않는다. 모든 결정을 자동으로 내린다.
2. **환경 감지 우선**: M1 전에 `bash lib/env-detect.sh`를 실행하여 사용 가능한 스택을 확인한다. 요청 스택이 사용 불가하면 가장 가까운 대안을 자동 선택한다.
3. **체크포인트 복구**: 파이프라인 시작 시 기존 체크포인트를 확인한다. 마지막 성공 마일스톤부터 재개한다.
4. **빌드 에러 자동 수정**: 패키지 API 변경, 타입 에러 등은 실제 설치된 버전의 타입 정의를 읽고 코드를 수정한다. 최대 3회 재시도.
5. **패키지 호환성**: `npm ls`, `pip list`, `dart pub deps` 등으로 실제 버전을 확인하고 해당 버전의 API에 맞게 코드를 생성한다.
...
```bash
bash lib/env-detect.sh
```

출력에서 사용 가능한 스택 목록을 파싱한다.
요청된 스택이 목록에 없으면:
- 가장 가까운 대안을 자동 선택 (예: Flutter 없으면 → Next.js 웹으로 전환)
- 사용자에게 알리지 않고 자동 결정
...
자동 품질 게이트 (모든 세대 동일, 사용자 개입 없음):
- 모든 섹션 비어있지 않음
- skills 목록에 harness-maker 포함 (부트스트래핑)
- hooks 경로가 commands/agents와 일관됨
- SessionStart/SessionStop 훅 설계 포함 (memory_persistence 교훈)
- Model routing 정책 포함 (cost_efficiency 교훈)
- Eval rubric 포함 (eval_coverage 교훈)
실패 시 M4 재실행 (최대 2회).
...
M6(audit)과 M8(smoke) 결과를 읽어서 **자동** 판정:
...
if audit.total_score >= config.perfection_threshold and smoke.passed:
    # 하네스 완성 → 프로덕트 파이프라인으로 (사용자 확인 없이)
    proceed_to_product()
elif gen >= config.max_generations:
    # 세대 한도 도달 → 현재 최선으로 진행
    proceed_to_product()
else:
    # 다음 세대로 (자동)
...
**사용자에게 묻지 않는다.** 판정은 숫자 기반으로 자동 결정된다.
